...
#   * vous utiliserez à cette occasion les briques standard de la programmation fonctionnelle disponible dans la bibliothèque standard de Python
# 3. un introduction aux décorateurs, des fonctions "qui modifient le comportement d'autres fonction"
# 4. la découverte des fonctionnalités spécifique des décorateurs en Python qui doit vous amener à une application utile en classe avec les élèves
###############################################################################
...
def forall_funct(pred, l):
    """TODO : une version résursive de forall, plus fonctionnelle, utilisant UNIQUEMENT empty, head et tail sans boucle for ni affectation. La fonction sera récursive"""
    pass
...
def forall_filter(pred, l):
    """TODO : une version de forall utilisant les fonctions standards filter et len. On pourra avoir besoin de transformer le résultat interméidiaire avec list()"""
    pass
...
# Même question que précédement, mais écrire maybe UNIQUEMENT avec des lambdas et
...
# même chose que précédement un décorateur uniquement avec des lambdas mais en faisant attention à n'appeller f QU'UNE SEULE FOIS
...
print("On ne doit voir que 2 fois le print dans la fonction de test (et pas 4 sans memoize)")
...
# Python propose une syntaxe ad hoc @decorator pour la manipulation des décorateurs. De plus, la bibliothèque standard propose quelques outils pour en faciliter la manipulation ainsi que quelques décorateurs très utiles. Voir :
#
