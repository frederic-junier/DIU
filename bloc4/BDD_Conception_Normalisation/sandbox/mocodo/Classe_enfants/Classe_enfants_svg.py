#!/usr/bin/env python
# encoding: utf-8
# Généré par Mocodo 2.3.7 le Wed, 27 May 2020 07:50:34
...
with codecs.open('Classe_enfants_geo.json') as f:
...
lines += u"""\n\n<!-- Association Herite -->"""
(x,y) = (cx[u"Herite"],cy[u"Herite"])
(ex,ey) = (cx[u"Classe_enfant"],cy[u"Classe_enfant"])
leg=straight_leg_factory(ex,ey,48,34,x,y,26,25,22+2*card_margin,15+2*card_margin)
...
(tx,ty)=offset(*leg.card_pos(False,shift[u"Herite,Classe_enfant"]))
...
(ex,ey) = (cx[u"Classe_parent"],cy[u"Classe_parent"])
leg=straight_leg_factory(ex,ey,49,34,x,y,26,25,22+2*card_margin,15+2*card_margin)
lines += u"""\n<line x1="%(ex)s" y1="%(ey)s" x2="%(ax)s" y2="%(ay)s" stroke="%(stroke_color)s" stroke-width="2"/>""" % {'ex': ex, 'ey': ey, 'ax': x, 'ay': y, 'stroke_color': colors['leg_stroke_color']}
(tx,ty)=offset(*leg.card_pos(False,shift[u"Herite,Classe_parent"]))
lines += u"""\n<text x="%(tx)s" y="%(ty)s" fill="%(text_color)s" font-family="Verdana" font-size="12">0,N</text>""" % {'tx': tx, 'ty': ty, 'text_color': colors['card_text_color']}
lines += u"""\n<g id="association-Herite">""" % {}
...
lines += u"""\n	<text x="%(x)s" y="%(y)s" fill="%(text_color)s" font-family="Verdana" font-size="12">Herite</text>""" % {'x': -19+x, 'y': -7.3+y, 'text_color': colors['association_cartouche_text_color']}
lines += u"""\n</g>""" % {}

lines += u"""\n\n<!-- Entity Classe_parent -->"""
(x,y) = (cx[u"Classe_parent"],cy[u"Classe_parent"])
lines += u"""\n<g id="entity-Classe_parent">""" % {}
lines += u"""\n	<g id="frame-Classe_parent">""" % {}
...
lines += u"""\n	</g>""" % {}
lines += u"""\n	<text x="%(x)s" y="%(y)s" fill="%(text_color)s" font-family="Verdana" font-size="12">Classe_parent</text>""" % {'x': -44+x, 'y': -16.3+y, 'text_color': colors['entity_cartouche_text_color']}
lines += u"""\n	<text x="%(x)s" y="%(y)s" fill="%(text_color)s" font-family="Verdana" font-size="12">id</text>""" % {'x': -44+x, 'y': 8.8+y, 'text_color': colors['entity_attribute_text_color']}
lines += u"""\n	<line x1="%(x0)s" y1="%(y0)s" x2="%(x1)s" y2="%(y1)s" stroke="%(stroke_color)s" stroke-width="1"/>""" % {'x0': -44+x, 'y0': 11+y, 'x1': -32+x, 'y1': 11+y, 'stroke_color': colors['entity_attribute_text_color']}
lines += u"""\n	<text x="%(x)s" y="%(y)s" fill="%(text_color)s" font-family="Verdana" font-size="12">description</text>""" % {'x': -44+x, 'y': 25.8+y, 'text_color': colors['entity_attribute_text_color']}
lines += u"""\n</g>""" % {}

lines += u"""\n\n<!-- Entity Classe_enfant -->"""
(x,y) = (cx[u"Classe_enfant"],cy[u"Classe_enfant"])
lines += u"""\n<g id="entity-Classe_enfant">""" % {}
lines += u"""\n	<g id="frame-Classe_enfant">""" % {}
...
lines += u"""\n	</g>""" % {}
lines += u"""\n	<text x="%(x)s" y="%(y)s" fill="%(text_color)s" font-family="Verdana" font-size="12">Classe_enfant</text>""" % {'x': -43+x, 'y': -16.3+y, 'text_color': colors['entity_cartouche_text_color']}
lines += u"""\n	<text x="%(x)s" y="%(y)s" fill="%(text_color)s" font-family="Verdana" font-size="12">id</text>""" % {'x': -43+x, 'y': 8.8+y, 'text_color': colors['entity_attribute_text_color']}
lines += u"""\n	<line x1="%(x0)s" y1="%(y0)s" x2="%(x1)s" y2="%(y1)s" stroke="%(stroke_color)s" stroke-width="1"/>""" % {'x0': -43+x, 'y0': 11+y, 'x1': -31+x, 'y1': 11+y, 'stroke_color': colors['entity_attribute_text_color']}
lines += u"""\n	<text x="%(x)s" y="%(y)s" fill="%(text_color)s" font-family="Verdana" font-size="12">description</text>""" % {'x': -43+x, 'y': 25.8+y, 'text_color': colors['entity_attribute_text_color']}
lines += u"""\n</g>""" % {}
lines += u'\n</svg>'

with codecs.open("Classe_enfants.svg", "w", "utf8") as f:
    f.write(lines)
safe_print_for_PHP(u'Fichier de sortie "Classe_enfants.svg" généré avec succès.')