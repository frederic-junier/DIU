#!/usr/bin/env python
# encoding: utf-8
# Généré par Mocodo 2.3.7 le Wed, 27 May 2020 07:55:00
...
with codecs.open('Classes_geo.json') as f:
...
leg=curved_leg_factory(ex,ey,39,34,x,y,26,25,22+2*card_margin,15+2*card_margin,-1.0)
...
(tx,ty)=offset(*leg.card_pos(shift[u"Herite,Classe,-1.0"]))
...
lines += u"""\n	</g>""" % {}
lines += u"""\n	<text x="%(x)s" y="%(y)s" fill="%(text_color)s" font-family="Verdana" font-size="12">Classe</text>""" % {'x': -20+x, 'y': -16.3+y, 'text_color': colors['entity_cartouche_text_color']}
lines += u"""\n	<text x="%(x)s" y="%(y)s" fill="%(text_color)s" font-family="Verdana" font-size="12">id</text>""" % {'x': -34+x, 'y': 8.8+y, 'text_color': colors['entity_attribute_text_color']}
lines += u"""\n	<line x1="%(x0)s" y1="%(y0)s" x2="%(x1)s" y2="%(y1)s" stroke="%(stroke_color)s" stroke-width="1"/>""" % {'x0': -34+x, 'y0': 11+y, 'x1': -22+x, 'y1': 11+y, 'stroke_color': colors['entity_attribute_text_color']}
...
with codecs.open("Classes.svg", "w", "utf8") as f:
    f.write(lines)
safe_print_for_PHP(u'Fichier de sortie "Classes.svg" généré avec succès.')