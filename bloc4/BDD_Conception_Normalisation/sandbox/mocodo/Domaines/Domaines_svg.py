#!/usr/bin/env python
# encoding: utf-8
# Généré par Mocodo 2.3.7 le Wed, 10 Jun 2020 05:46:35
...
with codecs.open('Domaines_geo.json') as f:
...
leg=straight_leg_factory(ex,ey,38,34,x,y,54,25,21+2*card_margin,15+2*card_margin)
...
(ex,ey) = (cx[u"Domaine"],cy[u"Domaine"])
leg=straight_leg_factory(ex,ey,38,34,x,y,51,25,21+2*card_margin,15+2*card_margin)
lines += u"""\n<line x1="%(ex)s" y1="%(ey)s" x2="%(ax)s" y2="%(ay)s" stroke="%(stroke_color)s" stroke-width="2"/>""" % {'ex': ex, 'ey': ey, 'ax': x, 'ay': y, 'stroke_color': colors['leg_stroke_color']}
...
(ex,ey) = (cx[u"Domaine"],cy[u"Domaine"])
leg=straight_leg_factory(ex,ey,38,34,x,y,43,25,22+2*card_margin,15+2*card_margin)
lines += u"""\n<line x1="%(ex)s" y1="%(ey)s" x2="%(ax)s" y2="%(ay)s" stroke="%(stroke_color)s" stroke-width="2"/>""" % {'ex': ex, 'ey': ey, 'ax': x, 'ay': y, 'stroke_color': colors['leg_stroke_color']}
...
(ex,ey) = (cx[u"LotBouteille"],cy[u"LotBouteille"])
leg=straight_leg_factory(ex,ey,47,76,x,y,34,25,22+2*card_margin,15+2*card_margin)
lines += u"""\n<line x1="%(ex)s" y1="%(ey)s" x2="%(ax)s" y2="%(ay)s" stroke="%(stroke_color)s" stroke-width="2"/>""" % {'ex': ex, 'ey': ey, 'ax': x, 'ay': y, 'stroke_color': colors['leg_stroke_color']}
(tx,ty)=offset(*leg.card_pos(False,shift[u"Stocké à,LotBouteille"]))
...
lines += u"""\n	<text x="%(x)s" y="%(y)s" fill="%(text_color)s" font-family="Verdana" font-size="12">prix</text>""" % {'x': -42+x, 'y': 17.8+y, 'text_color': colors['entity_attribute_text_color']}
lines += u"""\n	<text x="%(x)s" y="%(y)s" fill="%(text_color)s" font-family="Verdana" font-size="12">couleur</text>""" % {'x': -42+x, 'y': 34.8+y, 'text_color': colors['entity_attribute_text_color']}
...
with codecs.open("Domaines.svg", "w", "utf8") as f:
    f.write(lines)
safe_print_for_PHP(u'Fichier de sortie "Domaines.svg" généré avec succès.')